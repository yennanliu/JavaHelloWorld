...
/** Ref : - https://github.com/yennanliu/til/blob/master/doc/pic/stream_map1.png */
import dev.bean.Car;
...
public class StreamTest1 {
   ...
   * GroupBy basic
   *
   * <p>Map<String, List<MyReport>> monthReportMap = myReport.stream()
  ...
  public void GroupByTest1() {
    ...
    Map<String, List<Car>> groupByResult =
        car_list.stream().collect(Collectors.groupingBy(x -> x.getBrand()));

    System.out.println(
        ">>> groupByResult = "
            + groupByResult); //  groupByResult = {BENZ=[Car{id=2, brand='BENZ', price=3000.0}],
                              // LEXUS=[Car{id=3, brand='LEXUS', price=2000.0}, Car{id=4,
                              // brand='LEXUS', price=1222.0}, Car{id=5, brand='LEXUS',
                              // price=9000.0}], TESLA=[Car{id=1, brand='TESLA', price=1000.0}]}
  ...
  /**
   * 1) groupingBy Car::getId 2) Collectors.groupingBy(Car::getIdAndBrand) : group by custom get
   * method
   */
  @Test
  public void GroupByTest2() {
    ...
    Map<Integer, List<Car>> groupByResult =
        car_list.stream().collect(Collectors.groupingBy(Car::getId));
    ...
    Map<String, List<Car>> groupByResult2 =
        car_list.stream().collect(Collectors.groupingBy(Car::getIdAndBrand));
