...
public class TeamService {
  ...
  public void addMember(Employee e) throws TeamException {
    ...
    // switch accept arg : byte/short/char/int/String/Enum/Enumeration
    switch (p.getStatus()) {
      case BUSY:
        throw new TeamException("member already belongs to some other team");
      case VOCATION:
    ...
    /**
     * *** NOTE : WE CAN'T WRITE LOGIC LIKE BELOW -> below cases may happen
     *
     * <p>1) for (p instanceof Architect && numOfArch >= 1) -> could be cases that instanceof
     * Architect is true -> but numOfArch >= 1 is false -> So, the logic will go to ` else if (p
     * instanceof Designer && numOfDes > 2) ` -> which is not what we want
     *
     * <p>2) consider case : -> there are 2 designers in team, now we want add 1 new Architect -> V1
     * (above) will work properly, while V2 (below) will be wrong
     */
