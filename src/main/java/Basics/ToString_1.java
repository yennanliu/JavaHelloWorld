 ...
 *
 *   1) When we print out one class's instance -> it actually calls the class's toString() method
 *
 *   2) Object's toString method (default) definition
 *      ->  public String toString() {
 *              return getClass().getName() + "@" + Integer.toHexString(hashCode());
 *         }
 *
 *   3) Some classes such String, Date, File ... already RE-WRITE objects' toString() method
 *      -> so when we call toString() within those class, it will return "actual" content
 *
 *   4) if we want to do above, we need to re-write toString() method as well
 *
 */
