 ...
 *
 * <p>1) Callable VS Runnable 1-1) call() method can have returned value 1-2) call() method can
 * throw exception (can be caught by external code) 1-3) Callable supports genetic type
 *
 * <p>2) Future interface - Can cancel, check... on Runnable, Callable tasks - FutureTask is the
 * only implemented class of Future - FutureTask implement Runnable, Callable on the same time, it
 * can not only run by Runnable thread, but also can as returned value (Future) of Callable
 */
...
public class CallableDemo1 {
  public static void main(String[] args) {
    ...
    // step 4) pass callable implemented class (NumThread) as input to FutureTask constructor, make
    // FutureTask instance
    FutureTask futureTask = new FutureTask(numThread);

    // step 5) pass FutureTask instance as input to Thread class, make Thread instance, and run
    // start() method
    // start the thread
