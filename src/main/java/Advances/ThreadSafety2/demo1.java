 ...
 *
 *  1) difference / same between synchronized and lock ?
 *
 *    - difference :
 *        - synchronized (method, code block) will release synchronized monitors automatically after running
 *        - lock : we need to manually run (lock.lock(), lock.unlock())
 *    - same :
 *        - both of them can solve thread safety
 *
 *  2) preference
 *      - Lock -> synchronized code block -> synchronized method
 *
 *  3) apps, classes... (before JDK 5.0) use synchronized (method, code block) mainly
 *     for cases after JDK 5.0, we use Lock more
 *
 *
