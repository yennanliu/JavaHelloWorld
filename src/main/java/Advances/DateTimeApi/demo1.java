...
public class demo1 {
   ...
   *
   * <p>1) 2 constructors - Date() : create an Date() instance of CURRENT TIME - Date(someTimeStamp)
   * : create an Date() instance of `given timestamp`
   *
   * <p>2) 2 methods - toString() : show current year, month, day, hour, min, sec - getTime(): get
   * current timeStamp (milliSecond) (e.g. time difference between `1980/1/1 0 hr 0 min 0 sec` and
   * now)
   *
   * <p>3) java.sql.Date class is for datetime objects in database - how to instantiate ? -
  ...
  public void test3() {
    ...
    // java.sql.Date date_1_ = (java.sql.Date) date1; // wrong, Date is not java.sql.Date 's parent
    // class

    /** NOTE here !!! */
    // so we get date1's timestamp via getTime() method, then put the output into java.sql.Date
    // constructor
