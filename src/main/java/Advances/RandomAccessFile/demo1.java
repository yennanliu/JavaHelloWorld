 ...
 *
 * <p>1) RandomAccessFile is directly below java.lang.Object class, implement DataInput, DataOutput
 * interface
 *
 * <p>2) RandomAccessFile can be BOTH input, and output flow (but have to init 2 java instances)
 *
 * <p>3) while RandomAccessFile DataOutput, 3-1) if output file not existed, RandomAccessFile will
 * automatically make a new one 3-2) if output file already existed, will OVERWRITE original file
 * (write from beginning)
 *
 * <p>4) can do "file insert" via RandomAccessFile (can add data to file with specific idx (not only
