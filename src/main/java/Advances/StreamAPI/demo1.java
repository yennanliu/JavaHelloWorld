 ...
 * Stream API demo 1
 *
 * <p>stream initiation op
 *
 * <p>1) Stream API : for data manipulation (deal with CPU) 2) Stream API VS collection ->
 * collection focus on data storage (deal with memory) 3) stream will NOT storage elements 4) stream
 * will NOT change source instance; they return result to the other new stream 5) stream op is
 * "lazy" operation, it will NOT be executed until result is called 6) stream op steps: -> step 1)
 * stream initiation -> step 2) stream op (filter, mapping..) -> step 3) terminate op
 *
 * <p>7) once terminated, stream result CAN NOT BE retrieved again
 */
public class demo1 {
