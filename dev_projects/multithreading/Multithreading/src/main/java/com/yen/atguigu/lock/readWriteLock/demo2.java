 ...
 * <p>In short,
 *
 * <p>1. 讀讀共享 2. 寫寫互斥 (thread unsafe) 3. 讀寫互斥 (thread unsafe)
 *
 * <p>https://en.wikipedia.org/wiki/Readers%E2%80%93writer_lock
 */
 ...
 * Conclusion :
 *
 * <p>1、讀取和讀取之間不互斥，因為讀取操作不會有線程安全問題
 *
 * <p>2、寫和寫之間互斥，避免一個寫操作影響另外一個寫操作，引發線程安全問題
 *
 * <p>3、讀取和寫入之間互斥，避免讀取操作的時候寫操作修改了內容，引發線程安全問題
 *
 * <p>-> 總結起來就是，多個Thread可以同時進行讀取操作，但是同一時刻只允許一個Thread進行寫入操作。
 */
class MyDomain3 {
...
class Mythread3_1 extends Thread {
  ...
  private final MyDomain3 myDomain3;
