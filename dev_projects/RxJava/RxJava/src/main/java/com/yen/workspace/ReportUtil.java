...
public class ReportUtil {
  ...
  public static List<TestResult> getTestResultBySerialNum(String serialNum) {
    ...
    return getReports().stream()
         ...
         * NOTE !!! flatMap syntax
         *
         * <p>1) flatMap(report -> report.getResults().stream()): Flattens the stream of reports
         * into a stream of test results.
         *
         * <p>2) result type of flatMap : Stream<TestResult> e.g. : Stream<TestResult> tmp =
         * getReports().stream().flatMap(report -> report.getResults().stream());
         */
